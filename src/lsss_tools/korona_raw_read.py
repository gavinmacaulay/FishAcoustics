...
def get_plankton(pid, raw, cat_names):
    ...
    data = []
    for time_bin, group in gp:
        counts, _ = np.histogram(group, bins=np.arange(-0.5, max_cat_id+1.5, 1))
        d = np.append(counts, time_bin.mid)
        data.append(d)

    col_names = [str(x) for x in np.arange(max_cat_id+1)]
